################################################################################
#' @description This script is an adjusted version of Pancho's old file VAMCM-DataCleaning009.R
#' As in the original, this cleans study database and creates an age-specific id variable that is used in the model input.
#' The model inputs don't have all covariates, just the ones that were used in the model.
#' Need to replicate this script in order to get id variable matched with all covariates.
#' @return Study database with same id variable as age-specific model inputs and all covariates included
################################################################################
#' Clear environment
rm(list = ls())
#' Libraries
...
## Downweighted Mozambique data point
MOZ2007 <- read.dta13(paste0('./data/pancho/92602_MOZINCAM_downweighted.dta'), nonint.factors = T)
################################################################################

## Hal
# Create record number
dat$RecNr <- 1:nrow(dat)
# Create factor of location before any data points are dropped or merged
# For use in idAux
dat$location_fac <- as.numeric(relevel(factor(dat$study_location), ref = 'nationwide'))
...
# Hal update
# Replace column value if it has the same column name
# The above code doesn't work well anymore because I added the location_fac column
for(i in 1:ncol(MOZ2007)){
  if(names(MOZ2007)[i] == names(dat)[i]){
    dat[dat$Refid == 92602, i] <-  MOZ2007[,i]
  }
}
rm(MOZ2007)
...
## Save a study_location character variable
dat$location_char <- dat$location
...
## Hal id with location factor that was generated before dropping any data points
dat$idAux <- paste(dat$ISO3, dat$year, dat$Refid, dat$location_fac,
                dat$age_lb, dat$age_ub, dat$sex, sep = '-')
...
studies$iso3 <- studies$ISO3
studies$ISO3 <- NULL
studies$year <- studies$Year
studies$Year <- NULL
...
## so this one should match better across age-specific study databases
## makes it easier to see if the same data points are used in multiple models
## The id and idAux variables are always updated when data points are merged/collapsed. 
## So ids for these will sometimes not match between age groups if they were collapsed by sex for one model and not the other.
# View(studies[,c("RecNr","id","idAux")])

write.csv(studies, paste0("./gen/recover-studies2019-5to19y/temp/studydb-old-allcovar_", ageSexSuffix,".csv", sep =""), row.names = FALSE)
