################################################################
# Harmonizes covariate names and scales in prediction database with study database names
################################################################

#' Output: new prediction database with same covariate names and scales as study database
...
predmain <- pred %>% select(all_of(c(v_pred_id_col, key$pred))) %>%
  ...
  mutate(value = case_when( # adjust values that are in different scales
    !(value %in% c("sab", "contraception_met")) ~ value,
    pred == "sab" ~ value/100,
    pred == "contraception_met" ~ (1-value)*100
  )
  ) %>%  
...
write.csv(pred_new, "./gen/update-old-studydb-covar/output/pred-adjusted.csv", row.names = FALSE)
