...
dat <- read.csv(paste0("./gen/update-natl-covar-for-old-studies/temp/studies-adjusted_", ageSexSuffix, ".csv",sep=""))
...
df_src <- df_src %>%
  ...
  mutate(source_type = ifelse( is.na(source_type) & variable %in% v_dhs_covar, "Survey", source_type)) %>%
